...
**Actualizado 2026-09-14 (estado vigente, ver historial de cambios en "Cambio de modelo" y "Costos y ejecución" más abajo):** Rutas: `/cotizaciones/:id`, `/cotizaciones/:id/archivo` (PDF, `StreamableFile`), `/proyectos/:proyectoId/cotizaciones`, `/tareas/:tareaId/cotizaciones[/activa]`, `POST /cotizaciones` (**multipart**, campo `archivo` opcional, PDF máx 10MB). Body: `{proyectoId, tareaId, proveedorId, montoTotal: number, moneda: Moneda, ivaIncluido: boolean}` — `tareaId` es **obligatorio** desde el 2026-08-20 (ya no existe la cotización general a nivel proyecto, ver "Cambio de modelo" más abajo), y **no existe** `GET /proyectos/:proyectoId/cotizaciones/activa` (se borró en el mismo cambio). Cada `POST` marca la versión anterior de esa misma tarea como `REEMPLAZADA` automáticamente (lógica server-side). Respuesta: `{id, proyectoId, tareaId, proveedorId, montoTotal: string /* Decimal */, moneda, ivaIncluido, estado, archivoPdfRuta}`. Errores: 404 `COTIZACION_NO_ENCONTRADA`/`COTIZACION_ACTIVA_NO_ENCONTRADA`/`PROYECTO_O_PROVEEDOR_NO_ENCONTRADO`, 422 `TAREA_NO_PERTENECE_AL_PROYECTO`.

### Órdenes de Compra (`/ordenes-compra`) — módulo completo (Fase 3)
...
- ~~**Bug real por arreglar: fecha por defecto de OC nueva usa `toISOString()`**~~ — **resuelto (2026-08-21)**, ver detalle en la sección de testing exhaustivo más abajo.
...
- **Tipo de cambio / conversión de moneda — resuelto (2026-08-19).** Implementado tal cual estaba previsto en este mismo punto (backend `TipoCambio`/`tipos-cambio/`, frontend `lib/tipos-cambio/`), con un ajuste de UX: la edición de las tasas no fue a `/configuracion` ni a Sectores — el usuario pidió tenerla "escondida" al final de la página de **Auditoría** (`SeccionTiposCambio`, ver "Costos y ejecución" más abajo para el detalle vigente de la tarjeta de costos — se llamó "Comprometido" y "Costos y rentabilidad" en versiones anteriores).
...
### Mejora post-Fase 4 — Costo/rentabilidad en la tarjeta "Comprometido" (2026-08-19) — HISTÓRICO, superado el 2026-09-11

**Todo lo que describen estas dos secciones (esta y "Cambio de modelo — Propuesta de Inversión" de abajo) fue revertido/reemplazado el 2026-09-11 — ver "Costos y ejecución" más abajo para el modelo vigente.** Se dejan sin borrar porque explican el porqué de decisiones que siguen siendo relevantes (ej. por qué el cálculo es 100% client-side, por qué se sacó "General del proyecto" de Cotizaciones), pero **ningún nombre de componente, campo o endpoint de estas dos secciones existe ya tal cual** — no copiar código de acá sin verificar primero contra `lib/proyectos/presentacion.ts` real.
...
- **Actualización 2026-08-19 (mismo día, selector de moneda)**: la tarjeta muestra la moneda actual arriba a la derecha (junto al título, que pasó de `page.tsx` a dentro de `TarjetaComprometido` para poder ponerlos en la misma fila) con un botón "Cambiar". El override manual de "Costo SEG" solo es **editable** cuando la moneda mostrada coincide con la de la cotización general (`resumen.costoSegEditable`) — en cualquier otra moneda se ve sin botón de editar.
- **Actualización 2026-08-19 (mismo día, conversión real en vez de excluir)**: la primera versión del selector solo dejaba ver montos que ya estaban en esa moneda (todo lo demás quedaba en 0/"—"), poco útil. Se reemplazó por conversión real usando `lib/tipos-cambio/` (ver contexto del backend, sección "Avance de pago por proyecto", para el modelo/endpoints `TipoCambio`) — `calcularResumenCostos` ahora recibe también `tasas: Map<Moneda, number>` (armado en `TarjetaComprometido` desde `useMapaTiposCambio()`) y convierte **todo** (costo aproximado, honorarios, costo SEG calculado, gastado, y el override manual) a la moneda elegida antes de sumar/restar — nunca se mezclan montos sin convertir. El botón "Cambiar" ahora cicla siempre entre las 3 monedas soportadas (`MONEDAS` de `lib/cotizaciones/presentacion.ts`), no solo entre las que ya tienen datos — así siempre se puede probar, aunque el proyecto hoy solo tenga actividad en una. La UI para que ADMIN edite las tasas (`SeccionTiposCambio`) vive al final de `/auditoria` (pedido explícito del usuario, no en una página nueva).
...
- ~~`components/cotizaciones/ModalCotizacion.tsx` → nuevo campo "Honorarios" (opcional), visible **solo** cuando el alcance elegido es "General del proyecto"~~ — **eliminado el 2026-08-20**, ver más abajo.
...
### Cambio de modelo — Propuesta de Inversión reemplaza a la cotización general (2026-08-20)

Mismo cambio de negocio documentado en `contexto-gestion-interna-backend.md` (sección "Modelo de dominio" y "Avance de pago por proyecto"): se separó la propuesta comercial a nivel proyecto (ahora `PropuestaInversion`) de las cotizaciones de proveedores por tarea (`Cotizacion`, `tareaId` ya obligatorio). Cambios de frontend:

- **Pestaña nueva "Propuesta de inversión"** en `app/(app)/proyectos/[id]/page.tsx`, ubicada justo al lado de "Resumen" (orden final: Resumen, Propuesta de inversión, Cotizaciones, Órdenes de Compra). Muestra `TablaPropuestasInversion` (costo total, ahorro mensual, meses, % SEG, honorarios ya calculados por el backend, estado, descarga de archivo) + botón "Nueva versión" → `ModalPropuestaInversion` (costo total aproximado, ahorro mensual, cantidad de meses, % SEG, moneda, archivo PDF/Word/imagen opcional). Módulo nuevo `lib/propuestas-inversion/{tipos,api,hooks,presentacion}.ts`, mismo patrón de `lib/cotizaciones/` (versionado, `FormData` a mano) salvo que acepta 5 tipos de archivo en vez de solo PDF (`TIPOS_ARCHIVO_PROPUESTA_INVERSION_ACEPTADOS`, array de mimetypes, no un string único como `TIPO_ARCHIVO_COTIZACION_ACEPTADO`).
- **"Tareas" se fusionó dentro de la pestaña "Cotizaciones"** (ya no es pestaña propia): arriba la tabla de tareas con "Nueva tarea", abajo la tabla de cotizaciones con "Nueva cotización" — mecánicamente trivial, `TablaTareas`/`ModalTarea` son los mismos componentes de siempre, solo cambió dónde se montan en `page.tsx`. Si el proyecto todavía no tiene tareas, el botón "Nueva cotización" no se muestra y se avisa con un mensaje ("Creá una tarea antes de cargar una cotización.") en vez del formulario.
- `components/cotizaciones/ModalCotizacion.tsx`: se sacó la opción "General del proyecto" del Select (ahora "Tarea", requerido, sin sentinela `GENERAL`) y el campo condicional "Honorarios". `components/cotizaciones/TablaCotizaciones.tsx` ya no agrupa con el grupo sintético `{id: null, nombre: "General del proyecto"}`, agrupa directo por `tareas`. `components/proyectos/ProveedoresInvolucrados.tsx` simplificado (ya no hace falta el ternario "General del proyecto").
- `lib/proyectos/presentacion.ts` (`calcularResumenCostos`/`obtenerMonedasDisponibles`): ahora reciben `propuestaActiva: PropuestaInversion | null` en vez de derivar `general` de las cotizaciones (`cotizaciones.find(c => c.tareaId === null...)`, que ya no puede pasar). Costo aproximado sale de `propuestaActiva.costoTotalAproximado`, honorarios de `propuestaActiva.honorarios` (ya viene calculado del backend, no se duplica la fórmula en el frontend). El resto (costo SEG calculado desde cotizaciones de tarea activas, gastado desde OC pagadas, margen de equipo) no cambió.
- `components/proyectos/TarjetaComprometido.tsx` ahora pide también `usePropuestasInversionDeProyecto(proyecto.id)` y deriva la activa con `encontrarPropuestaActiva()`. Mensaje de estado vacío actualizado a "Sin propuesta ni cotizaciones activas".
...
### Costos y ejecución — reemplaza Propuesta de Inversión (2026-09-11, modelo vigente)

Mismo cambio de negocio documentado en `contexto-gestion-interna-backend.md` (sección "Modelo de dominio" → "Actualización 2026-09-11"). El usuario decidió sacar `PropuestaInversion` por completo: la tarjeta de costos pasa a calcular todo a partir de Cotizaciones de tarea y OC pagadas, sin depender de una entidad comercial separada a nivel proyecto ni de un override manual. Cambios reales (commit `601d03f`):

- **Se borró la pestaña "Propuesta de inversión"** de `app/(app)/proyectos/[id]/page.tsx` — las tabs quedaron en **Resumen, Cotizaciones, Órdenes de Compra** (3, no 4). Se borraron `ModalPropuestaInversion.tsx`, `TablaPropuestasInversion.tsx` y todo `lib/propuestas-inversion/`.
- **`components/cotizaciones/ModalCotizacion.tsx`** ganó el checkbox **"Incluye IVA"** (`ivaIncluido: boolean`, default `false`) — es el dato que reemplaza a `honorarios`/`PropuestaInversion` para poder calcular montos netos.
- **La tarjeta se renombró "Costos y ejecución"** (antes "Comprometido"/"Costos y rentabilidad") — `components/proyectos/TarjetaComprometido.tsx` quedó reescrito por completo. Ya no muestra Costo aproximado/Honorarios/Margen de equipo (dependían de la propuesta) ni tiene edición manual de Costo SEG — ahora solo dos números: **Costo SEG** y **Ejecución**, siempre netos de IVA, más un **desglose por tarea/proveedor** de cotizado vs. pagado.
- **`lib/proyectos/presentacion.ts` reescrito** (ver el archivo real para la fórmula exacta, esto es un resumen):
  - `calcularResumenCostos(cotizaciones, ordenesCompra, tasas, monedaSeleccionada?)` → `{moneda, monedasDisponibles, costoSeg, ejecucion} | null` (`null` si no hay ninguna cotización activa ni OC pagada en ninguna moneda). **Cambió la firma**: ya no recibe `proyecto`/`propuestaActiva`, solo `cotizaciones`/`ordenesCompra`/`tasas`/moneda opcional.
  - `costoSeg` = suma de `montoTotal` **neto de IVA** de las cotizaciones de tarea `ACTIVA`, convertido a la moneda elegida. `ejecucion` = suma de `monto` **neto de IVA** de las OC en estado `PAGADO`, convertido igual.
  - Neto de IVA usa una tasa **fija** `TASA_IVA_URUGUAY = 0.22` (no hay tasa configurable en ningún lado del sistema): para Cotizacion, si `ivaIncluido` divide por `1.22`; para OrdenCompra, solo divide si `pagaIva && ivaIncluido` a la vez (si no paga IVA, o si paga IVA pero el monto no lo incluye — se agrega aparte al pagar —, el monto ya es neto).
  - `calcularDesglosePorTarea(tareas, cotizaciones, ordenesCompra, tasas, moneda)` (nueva) → por cada tarea, lista de `{proveedorId, cotizado, pagado}` (ambos netos y convertidos) para los proveedores con actividad en esa tarea.
  - Conversión de moneda (pivote UYU vía `useMapaTiposCambio()`) y el selector "Cambiar" que cicla entre las 3 monedas soportadas **no cambiaron** respecto a la versión anterior.
- `BarraCostosProyecto` (gráfico) ajustado a los 2 valores nuevos (`costoSeg`/`ejecucion`) en vez de 3.
- `lib/auditoria/presentacion.ts` perdió las entradas de acciones de Propuesta de Inversión.
- Verificado `tsc --noEmit`/`eslint` sin errores en ambos repos. **No verificado visualmente en navegador** (misma limitación de siempre, ver [[feedback_claude_in_chrome_no_conecta]]).
- **Gotcha de despliegue real de este mismo cambio**: la migración de backend asociada (borra `propuestas_inversion`, agrega `iva_incluido`) quedó sin aplicar a mano en Railway durante 3 días (11 al 14/09) — producción devolvió `500` en cualquier pantalla que trajera Cotizaciones. Resuelto el 2026-09-14 con `railway ssh --service backend` + `npx prisma migrate deploy` (ver detalle completo en `contexto-gestion-interna-backend.md`, sección "Comandos de Railway"). **Lección para la próxima vez que se pushee un cambio de schema**: aplicar la migración a producción el mismo día del push, no dejarla para "después".
...
Pase de testing completo pedido por el usuario ("probá absolutamente todo"), ignorando a propósito bugs que requieran un usuario malicioso o técnico. Claude in Chrome **volvió a no conectar** (mismo problema de siempre, en ninguna sesión del proyecto conectó nunca) — la verificación siguió siendo 100% no visual: ~108 aserciones de API real contra el backend (cubre todo lo que la UI dispara) más lectura exhaustiva de código de `lib/**/presentacion.ts` y de los componentes, hecha por sub-agentes en paralelo con instrucciones de distinguir "confirmado en código" de "sospecha sin confirmar visualmente".

**Bug real encontrado — resuelto (2026-08-21):**
- ~~**`components/ordenes-compra/FormularioOrdenCompra.tsx:76`** — la fecha por defecto de una OC nueva se calcula con `new Date().toISOString().slice(0, 10)`, exactamente el anti-patrón que `contexto-gestion-interna-backend.md` prohíbe por escrito (Uruguay es UTC-3, `toISOString()` corre a UTC y adelanta el día). Reproducible todos los días entre las 21:00 y las 23:59 hora Uruguay.~~ — **arreglado**: nueva función `obtenerFechaLocalDeHoy()` en `lib/ordenes-compra/presentacion.ts` (arma el string `YYYY-MM-DD` a mano con `getFullYear()/getMonth()/getDate()` locales, sin pasar por UTC), usada como default de `fecha` en el formulario. No hace falta tocar el date-picker nativo aparte: el input sigue siendo un string plano, react-hook-form no lo reconstruye vía `Date`.

**Hallazgos menores — todos resueltos (2026-08-21):**
- ~~`lib/auditoria/presentacion.ts` (`ACCIONES_AUDITORIA`/`ENTIDADES_AUDITORIA`, hardcodeadas a mano) no incluyen `"ACTUALIZAR_TIPO_CAMBIO"` ni `"TipoCambio"`, ni `"RECALCULAR_COSTO_SEG_PROYECTO"`~~ — agregadas las tres, verificadas una a una contra `src/auditoria/acciones-auditoria.constantes.ts` del backend (sin gaps restantes).
- ~~`components/ui/Modal.tsx` no tiene `max-h-*`/`overflow-y-auto` en el panel~~ — el panel ahora es `flex max-h-[90vh] flex-col`, con el header fijo y el body (`{children}`) en su propio `overflow-y-auto`.
- ~~`app/(app)/ordenes-compra/[id]/page.tsx`: `mutacionEnCurso` no cubría los botones que abren modal (`rechazar`/`observar-pago`/`anular`)~~ — los tres ahora llevan `disabled={mutacionEnCurso}` igual que `enviar`/`aprobar`/`confirmarPago`.
- ~~`components/proyectos/TarjetaComprometido.tsx` + `lib/tipos-cambio/hooks.ts`: el guard de carga de la tarjeta no esperaba la query de tipos de cambio~~ — se agregó `useTiposCambio()` (además de `useMapaTiposCambio()`) y `!tiposCambio.data` al guard de carga, junto a `cotizaciones`/`ordenesCompra`/`propuestas.data`.
