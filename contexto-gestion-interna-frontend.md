...
- **Bug real por arreglar: fecha por defecto de OC nueva usa `toISOString()`** (encontrado en el testing exhaustivo del 2026-08-20/21, ver esa sección más abajo) — `FormularioOrdenCompra.tsx:76` adelanta un día entre las 21:00 y las 23:59 hora Uruguay. Contradice la regla de fechas que el propio proyecto documentó por escrito (ver `contexto-gestion-interna-backend.md`, sección "Manejo de fechas").
...
**Bug real encontrado, pendiente de arreglar:**
- **`components/ordenes-compra/FormularioOrdenCompra.tsx:76`** — la fecha por defecto de una OC nueva se calcula con `new Date().toISOString().slice(0, 10)`, exactamente el anti-patrón que `contexto-gestion-interna-backend.md` prohíbe por escrito (Uruguay es UTC-3, `toISOString()` corre a UTC y adelanta el día). **Reproducible todos los días entre las 21:00 y las 23:59 hora Uruguay**: cualquier empleado que cargue una OC nueva en ese horario ve el campo "Fecha" precargado con el día siguiente, no con hoy. Es el único lugar del frontend con este patrón (`grep -rn "toISOString"` no encuentra otro). Arreglo: construir la fecha local igual que ya hace el backend (`ahora.getFullYear()/getMonth()/getDate()`, sin pasar por UTC), no solo en el valor por defecto sino confirmar que el date-picker nativo tampoco reintroduzca el problema al reformatear.

**Hallazgos menores, no urgentes:**
- `lib/auditoria/presentacion.ts` (`ACCIONES_AUDITORIA`/`ENTIDADES_AUDITORIA`, hardcodeadas a mano) no incluyen `"ACTUALIZAR_TIPO_CAMBIO"` ni `"TipoCambio"`, ni `"RECALCULAR_COSTO_SEG_PROYECTO"` — features agregadas el 2026-08-19/20 después de armar esa lista. Los registros igual aparecen en `/auditoria` sin filtrar, pero un ADMIN no puede aislarlos por el `<select>` de filtro. Mismo gotcha que ya advierte el comentario del propio archivo.
- `components/ui/Modal.tsx` no tiene `max-h-*`/`overflow-y-auto` en el panel — en un formulario largo (ej. `ModalProveedor`, 7 campos) en una pantalla baja (mobile en landscape, tablets chicas), el botón de submit podría quedar recortado sin scroll para llegar a él. No confirmado visualmente.
- `app/(app)/ordenes-compra/[id]/page.tsx`: el estado `mutacionEnCurso` que deshabilita botones solo cubre `enviar`/`aprobar`/`confirmarPago`, no los botones que abren modal (`rechazar`/`observar-pago`/`anular`, líneas ~226/232/251) — un doble click rápido entre dos transiciones distintas podría disparar dos mutaciones concurrentes sobre la misma OC; el backend rechaza una con 409 `TRANSICION_INVALIDA` pero el usuario ve un error confuso sin contexto.
- `components/proyectos/TarjetaComprometido.tsx` + `lib/tipos-cambio/hooks.ts`: `useMapaTiposCambio()` devuelve un `Map` vacío (no `undefined`) mientras `GET /tipos-cambio` todavía no resolvió, y el guard de carga de la tarjeta no espera esa query en particular — en un proyecto con actividad en USD/EUR, si esa request tarda más que las otras 3 (cotizaciones/OC/propuestas), la tarjeta podría pintar un instante con tasa 1:1 (montos sin convertir) antes de re-renderizar con la tasa real. No confirmado visualmente.
- Backend: 6 errores de `eslint` (solo `prettier/prettier`, formato) en archivos no relacionados con este repo — ver el detalle en `contexto-gestion-interna-backend.md`.
