...
- `npx tsc --noEmit` y `npx eslint .` sin errores en ningún punto de las dos fases.
...
- **Seguimos sin poder probar en un navegador real** — la extensión Claude in Chrome no conectó en ninguna sesión hasta ahora. Toda la verificación de Fase 1 y Fase 2 fue por `tsc`/`eslint`/`curl` directo a la API, nunca clickeando la UI real. Sigue pendiente la pasada visual completa.
...
- **Verificación visual en navegador real** sigue pendiente desde Fase 0 — la extensión Claude in Chrome no conectó en ninguna sesión hasta ahora. Todo lo construido en Fase 0/1/2 se probó por `tsc`/`eslint`/`curl`, nunca clickeando la UI.
...
### ⬜ Fase 3 — Órdenes de Compra (el núcleo, siguiente paso)
Listado con filtros, formulario de creación (multipart, reflejar las validaciones del backend: proveedor debe coincidir con cotización, monto no debe exceder), detalle con badges de estado (implementar el mapeo `EstadoOC`→tono de `Insignia` propuesto arriba), botones de transición condicionados por rol+sector+estado+tabla de transiciones, historial, hilo de comentarios (con el aviso de que comentar puede cambiar el estado automáticamente — refrescar la OC después de comentar), adjuntar/reemplazar factura. Necesita `TipoOC`, `FormaPago`, `EstadoOC` completos.

### ⬜ Fase 4 — Auditoría
Tabla de solo lectura con filtros (`accion`, `entidad`, `usuarioEmail`), `RequiereRol(["ADMIN"])`. Revisar `ACCIONES_AUDITORIA` del backend para el `<select>` de filtro por acción.
